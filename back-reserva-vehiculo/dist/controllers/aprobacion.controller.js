...
exports.denegarReserva = exports.confirmarReserva = void 0;
...
const confirmarReserva = async (req, res) => {
    ...
    try {
      ...
      `);
        ...
        const { usuario, fecha_inicio, fecha_fin, marca, modelo, matricula, vehiculo_pkid, } = reserva.recordset[0];
        /* 2️⃣ RESERVA → OCUPADO */
        await db.request()
            .input('token', token)
            .query(`
      ...
      `);
        ...
        await (0, mail_service_server_1.enviarCorreoReservaAprobada)({
            usuario,
...
/* ============================
   DENEGAR RESERVA
============================ */
const denegarReserva = async (req, res) => {
    const { token } = req.query;
    if (!token || typeof token !== 'string') {
        return res.status(400).send('Token inválido');
    }
    ...
    try {
      ...
      `);
        /* 5️⃣ CORREO AL SOLICITANTE */
        await (0, mail_service_server_1.enviarCorreoReservaDenegada)({
            usuario,
            vehiculo: { marca, modelo, matricula },
            fechaInicio: fecha_inicio.toISOString(),
            fechaFin: fecha_fin.toISOString(),
...
exports.denegarReserva = denegarReserva;
